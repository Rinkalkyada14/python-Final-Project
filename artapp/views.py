...
def forgot_password(request):
    if request.method == "POST":
        try:
            ...
            querystring = {
                "authorization": "csMwYan580kxUQXGPl9A2NvZHTCjLDW4hug3KpR6FzIdV7fyoqqlCnrEOpRFMWtdxZ7a96LBXSz034Tf",
                "variables_values": str(otp),
                "route": "otp",
                "numbers": str(user.mobile),
            }
...
def profile(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            print(" i am here.....", e, print(request.FILES))
        user.save()
        request.session["profile_pic"] = user.profile_pic.url
...
def artist_profile(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "artist-profile.html",
            {"artist": artist, "artist_profile": artist_profile},
        )
    else:
...
def book_artist(request, pk):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "book-artist.html", {"artist_profile": artist_profile, "msg": msg}
        )
    else:
